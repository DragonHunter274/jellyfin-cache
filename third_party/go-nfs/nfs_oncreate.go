...
func onCreate(ctx context.Context, w *response, userHandle Handler) error {
	...
	if how == createModeUnchecked || how == createModeGuarded {
		...
		// read createverf3 (verifier consumed but not stored; we use guarded semantics)
		var verf [8]byte
		if err := xdr.Read(w.req.Body, &verf); err != nil {
			return &NFSStatusError{NFSStatusInval, err}
		}
		// Treat exclusive as guarded (create only if not exists) with no attribute changes.
		// Full verifier-based idempotency is not implemented.
		how = createModeGuarded
